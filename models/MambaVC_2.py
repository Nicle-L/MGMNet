...
class Fuse_Block(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(self.output_dim, self.output_dim*2, 1, 1, 0, bias=True)
        self.conv2 = nn.Conv2d(self.output_dim*2, self.output_dim, 1, 1, 0, bias=True)
...
class MAMBAVC(CompressionModel):
    def __init__(self, depths=[2, 2, 2, 2], head_dim=[8, 16, 32, 32 ,16, 8], drop_path_rate=0.1, N=128, M=320, num_slices=5, max_support_slices=5,
        ...
        self.m_down1 = [Fuse_Block(input_dim=N * 2, output_dim=N*2, head_dim=self.head_dim[0],
                        ...
                        for i in range(depths[0])] + [down_sampling(N * 2, N * 2)]
        ...
        self.m_down2 = [Fuse_Block(input_dim=N * 2, output_dim=N*2, head_dim=self.head_dim[1],
                                 ...
                                 norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
                                 ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
                        for i in range(depths[1])] + [down_sampling(N * 2, N * 2)]
        ...
        self.m_down3 = [Fuse_Block(input_dim=N * 2, output_dim=N*2, head_dim=self.head_dim[2],
                                 ...
                                 norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
                                 ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
                        for i in range(depths[2])] + [down_sampling(N * 2, M)]

        self.g_a = nn.Sequential(
            *[down_sampling(3, N*2)] + self.m_down1 + self.m_down2 + self.m_down3)
        ...
        self.ha_down1 = [Fuse_Block(input_dim=N * 2, output_dim=N*2, head_dim=32,
                                  ...
                                  norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
                                  ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
                         for i in range(depths[3])] + [down_sampling(N*2, 192)]

        self.h_a = nn.Sequential(
            *[down_sampling(M, N*2)] + self.ha_down1
        )
        ...
        self.hs_up1 = [Fuse_Block(input_dim=N * 2, output_dim=N*2, head_dim=32,
                                ...
                                norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
                                ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
                       for i in range(depths[0])] + [up_sampling(N*2, M)]

        self.h_mean_s = nn.Sequential(
            *[up_sampling(192, N*2)] + self.hs_up1
        )
        ...
        self.h_scale_s = nn.Sequential(
            *[up_sampling(192, N*2)] + self.hs_up2
        )
        ...
        self.m_up1 = [Fuse_Block(input_dim=N * 2, output_dim=N*2, head_dim=self.head_dim[3],
                               ...
                               norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
                               ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
                      for i in range(depths[1])] + [up_sampling(N*2, N*2)]
        ...
        self.m_up2 = [Fuse_Block(input_dim=N * 2, output_dim=N*2, head_dim=self.head_dim[4],
                               ...
                               norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
                               ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
                      for i in range(depths[2])] + [up_sampling(N*2, N*2)]
        ...
        self.m_up3 = [Fuse_Block(input_dim=N * 2, output_dim=N*2, head_dim=self.head_dim[5],
                               ...
                               norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
                               ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
                      for i in range(depths[3])] + [up_sampling(N*2, 3)]

        self.g_s = nn.Sequential(*[up_sampling(M, N*2)] + self.m_up1 + self.m_up2 + self.m_up3)
...
# class MAMBAVC(CompressionModel):
#     def __init__(self, depths=[2, 2, 9, 2], drop_path_rate=0.1, N=128, M=320, num_slices=5, max_support_slices=5,
#                  **kwargs):
#         super().__init__(entropy_bottleneck_channels=N)
#         self.depths = depths
#         self.window_size = 8
...
#
#         self.ha_down1 = [VSSBlock(hidden_dim=2 * N, drop_path=dpr[i + depths[0] + depths[1] + depths[2]],
#                                   use_checkpoint=False,
#                                   norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
#                                   ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
...
#
#         self.m_up3 = [VSSBlock(hidden_dim=2 * N, drop_path=dpr[i + depths[0] + depths[1] + depths[2]],
#                                use_checkpoint=False,
#                                norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
#                                ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
...
if __name__ == "__main__":
    ...
    flops, params = profile(MAMBAVC, (input_image,))
